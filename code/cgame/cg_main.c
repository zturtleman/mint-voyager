...
static cvarTable_t cgameCvarTable[] = {
	...
	{ &cg_fov, "cg_fov", "80", CVAR_ARCHIVE, RANGE_FLOAT(1, 160) },
	{ &cg_weaponFov, "cg_weaponFov", "80", CVAR_ARCHIVE, RANGE_FLOAT( 0, 160 ) },
...
static void CG_RegisterSounds( void ) {
	...
	if ( cgs.gametype >= GT_TEAM || cg_buildScript.integer ) {
		...
		cgs.media.captureYourTeamSound = trap_S_RegisterSound( "sound/voice/computer/misc/scored.mp3", qtrue );
		cgs.media.captureOpponentSound = trap_S_RegisterSound( "sound/voice/computer/misc/scored_e.mp3", qtrue );

		cgs.media.returnYourTeamSound = trap_S_RegisterSound( "sound/voice/computer/misc/returned_e.mp3", qtrue );
		cgs.media.returnOpponentSound = trap_S_RegisterSound( "sound/voice/computer/misc/returned.mp3", qtrue );
		...
		if ( cgs.gametype == GT_CTF || cg_buildScript.integer ) {
			...
			cgs.media.enemyTookYourFlagSound = trap_S_RegisterSound( "sound/voice/computer/misc/stolen_e.mp3", qtrue );
			cgs.media.yourTeamTookEnemyFlagSound = trap_S_RegisterSound( "sound/voice/computer/misc/stolen.mp3", qtrue );
		}
		...
		if ( cgs.gametype == GT_1FCTF || cgs.gametype == GT_CTF || cg_buildScript.integer ) {
			// PORTNOTE: haveflag.mp3 is set on the flag item. this would cause it to play twice.
			//cgs.media.youHaveFlagSound = trap_S_RegisterSound( "sound/voice/computer/misc/haveflag.mp3", qtrue );
	...
	cgs.media.talkSound = trap_S_RegisterSound( "sound/interface/communicator.wav", qfalse );
	cgs.media.landSound = trap_S_RegisterSound( "sound/player/land1.wav", qfalse);

	cgs.media.hitSound = trap_S_RegisterSound( "sound/feedback/hit.wav", qfalse );
	cgs.media.hitSoundHighArmor = trap_S_RegisterSound( "sound/feedback/shieldhit.wav", qfalse );
	cgs.media.hitSoundLowArmor = cgs.media.hitSoundHighArmor;
	cgs.media.hitSoundPierceArmor = trap_S_RegisterSound( "sound/feedback/shieldpierce.wav", qfalse );
...
static void CG_RegisterGraphics( void ) {
	...
	if ( cgs.gametype == GT_CTF || cg_buildScript.integer ) {
		...
		cgs.media.redFlagBaseModel = trap_R_RegisterModel( "models/powerups/trek/flag_base.md3" );
		cgs.media.blueFlagBaseModel = cgs.media.redFlagBaseModel;

#ifdef MISSIONPACK
		cgs.media.neutralFlagBaseModel = cgs.media.redFlagBaseModel;
#endif
	}
...
void CG_Ingame_Init( int serverMessageNum, int serverCommandSequence, int maxSplitView, int playerNum0, int playerNum1, int playerNum2, int playerNum3 ) {
	...
	for (i = 0; i < CG_MaxSplitView(); i++) {
		cg.localPlayers[i].weaponSelect = DEFAULT_WEAPON;
	}
